...
func test_005_socketio_events():
	...
	if stream:
		...
		# Send an event from Mock Server: Type 2 (Event), Array payloads: ["chat_message", {"user":"admin", "text":"hello world"}]
		# Format according to standard Socket.io packets over WebSocket
		var mock_event_payload = "2[\"chat_message\", {\"user\":\"admin\", \"text\":\"hello world\"}]"
